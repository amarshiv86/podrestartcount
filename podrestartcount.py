...
class PodsList:
    ...
    def __init__(self, auth_method="local", namespace=None):
        ...
        if auth_method == 'local':
            ...
            """print(config.list_kube_config_contexts()[1])
...
class Report:
    ...
    def export_restarting_containers_report(self):
        ...
        cluster_name = config.list_kube_config_contexts()[1]['context']['cluster']
        print(cluster_name)
        csv_name = f'/Users/amar.singh/script/restarting-pods-report copy/src/podrestartcount.csv'
        path = '/Users/amar.singh/script/restarting-pods-report copy/src/podrestartcount.csv'
        print(os.path.isfile(path))
        print(f'Exporting {csv_name}')
        if os.path.isfile(path) == True:
            print("File already exist, appending to new rows")
            with open(csv_name, mode='a', encoding="utf-8") as exported_report:
                ...
                for container_obj in self.restarting_containers_report['restarting_containers']:
                    report_writer.writerow([cluster_name,
                                            container_obj['pod'],
                                            container_obj['name'],
            ...
            print("File doesn't exist, Creating new csv file")
            with open(csv_name, mode='w', encoding="utf-8") as exported_report:
                report_writer = csv.writer(exported_report, \
                                        delimiter=',',
                                        quotechar='"',
                                        quoting=csv.QUOTE_MINIMAL)
                report_writer.writerow(['Cluster','Pod', 'Container',
